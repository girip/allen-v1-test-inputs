...
base_lgn_stimulus_save_path = '../filternet/my_results'
...
for flash_time in flash_duration:
    for flash_pixel_value in flash_intensity:
        ...
        lgn_stimulus_path = base_lgn_stimulus_save_path + '/single_flash_stimulus/{}_ms_flash_duration/{}_flash_pixel_value_{}_by_{}/spikes.h5'.format(
            flash_time, flash_pixel_value, res_x, res_y)
        ...
        if os.path.isfile(configs_save_path) and os.path.isfile(lgn_stimulus_path) and not os.path.isfile(results_save_path + '/log.txt'):
